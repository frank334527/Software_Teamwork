...
- 启动时必须校验 PostgreSQL、Redis、file client、AI Gateway 配置和监听地址；Knowledge 服务配置是可选项，仅在生成请求要求检索上下文时使用；`DOCUMENT_PANDOC_PATH` 和 `DOCUMENT_LIBREOFFICE_PATH` 是富 DOCX 工具链预留配置，当前 host-run 基线不要求安装对应 CLI。
