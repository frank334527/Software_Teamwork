...
6. 本地 Docker 拉取慢、镜像源异常或 Compose 启动卡住时，阅读 [Docker 基础设施镜像拉取环境](runbooks/docker-image-pull-environment.md)。中国大陆网络优先保留 `deploy/.env.example` 的镜像 rewrite，并用 `python3 scripts/check_docker_environment.py --profile all --clean-env` 检查。
