...
| 宿主机 Go 模块下载 | `GOPROXY` + `GOSUMDB` env | `GOPROXY=https://goproxy.cn,direct`；`GOSUMDB=sum.golang.google.cn` | 已固定 | `deploy/.env.example` 默认写入宿主机 Go module 镜像和 checksum DB，用于 `dev-up.sh` 的 goose migration 以及 `run-backend.sh` 的 Go 服务 `go run`；这不是 Docker registry，也不是 Knowledge runtime uv 的 `UV_DEFAULT_INDEX`。 |
